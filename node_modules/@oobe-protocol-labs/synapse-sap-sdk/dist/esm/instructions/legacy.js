...
export class LegacyMemoryModule {
     ...
     * @returns {Promise<TransactionInstruction>} Transaction instruction
     */
    async appendMemoryChunk(ctx) {
        return this.program
            .methods.appendMemoryChunk(ctx.sequence, Array.from(ctx.encryptedData), ctx.nonce, ctx.totalFragments, ctx.fragmentIndex, ctx.compression, ctx.epochIndex)
            .accounts({
            wallet: ctx.wallet,
            agent: ctx.agent,
            vault: ctx.vault,
